...
#Create csv files with the unique input
...
#load in the dataset and wind park polygon
Visual_data_Gulls_2023 <- read_excel("/Users/constantijn/Desktop/2025_Birds/Data/Data_RAW/Visual_data_Gulls.xlsx", 
                                           sheet = "DATA 2023")
...
# Define list the folder with the CSV files
...
# putting the coordinates back in
...
#calculate the time and temperature changes
...
# Calculate the largest temperature drop per file
...
# Combine all indicators and put in the conditions for feeding events
...
# Calculating the proportion of total events
...
#filtering the behaviors per file
...
#binding both dataframes together and linear model
...
#calculating mean, min, max of the extensive maneuvering
summary_stats_ExMan_inPark <- files_ExMan %>%
  group_by(year, inPark) %>%
  summarise(
    mean_count = mean(count_ExMan_inPark),
    min_count = min(count_ExMan_inPark),
    max_count = max(count_ExMan_inPark),
    .groups = 'drop'
  )
...
#transforming windpark and tracker to the same CRS
